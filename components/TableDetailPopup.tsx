...
import { X, Coins, Bell, RotateCcw, Clock, Check, AlertTriangle, Timer, CreditCard } from "lucide-react";
...
export default function TableDetailPopup({ tableId, onClose }: { tableId: number; onClose: () => void }) {
  ...
  const toggleOrderStatus = (id: string) => {
    setOrders(prev => prev.map(order => {
      ...
      if (order.status === "입금 확인 대기") return { ...order, status: "준비 중" };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-0 sm:p-4" onClick={onClose}>
      <div className={`flex flex-col md:flex-row transition-all duration-500 w-full h-full sm:h-[90vh] max-w-[1400px] ${(isEditTokenOpen || isResetOpen) ? "md:-translate-x-[5vw]" : ""}`} onClick={(e) => e.stopPropagation()}>
        ...
        <motion.div className="relative flex-1 bg-[#1e293b] sm:rounded-[2rem] shadow-2xl border border-white/10 flex flex-col overflow-hidden z-10">
          ...
          <div className="px-6 py-4 sm:px-10 sm:py-6 border-b border-white/5 flex justify-between items-center bg-slate-800/40">
            <div className="flex items-center gap-4 sm:gap-12">
              ...
              <div className="flex items-center gap-4 sm:gap-10 border-l border-white/10 pl-4 sm:pl-10">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] sm:text-[12px] text-yellow-600 font-bold uppercase tracking-widest">토큰</span>
            ...
            <div className="space-y-4">
              ...
              {Object.keys(groupedOrders).map((time) => (
                <div key={time} className="bg-[#0f172a]/40 rounded-2xl p-4 border border-white/5">
                  <div className="text-[13px] text-slate-600 font-mono mb-3 border-b border-white/5 pb-2">주문시간: {time}</div>
                    ...
                    {groupedOrders[time].map((order: Order) => (
                      <div key={order.id} className="flex justify-between items-center bg-white/5 p-3 sm:p-4 rounded-xl border border-transparent">
                        ...
                        <button 
                          ...
                          className={`px-4 py-2 sm:px-6 sm:py-3 rounded-xl font-black text-xs sm:text-sm transition-all flex items-center gap-2 ${
                            order.status === "입금 확인 대기" ? "bg-emerald-600 text-white" : order.status === "제공 완료" ? "bg-slate-700 text-slate-500" : "bg-orange-500 text-white"
                          }`}
                        >
                          {order.status === "입금 확인 대기" && <CreditCard size={14} />}
                          {order.status === "입금 확인 대기" ? "입금 확인" : order.status}
                        </button>
                      </div>
                    ))}
                  </div>
          ...
          <div className="p-4 sm:p-8 bg-slate-900/40 border-t border-white/5 flex flex-col sm:flex-row gap-3">
            <button onClick={() => { setIsResetOpen(false); setIsEditTokenOpen(!isEditTokenOpen); }} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white py-4 sm:py-6 rounded-2xl font-black text-lg sm:text-xl flex items-center justify-center gap-3 active:scale-95">
              <Coins size={20} className="text-yellow-500" /> 토큰 수량 수정
            </button>
            <button onClick={() => { setIsEditTokenOpen(false); setIsResetOpen(!isResetOpen); }} className="flex-1 bg-orange-600 hover:bg-orange-500 text-white py-4 sm:py-6 rounded-2xl font-black text-lg sm:text-xl flex items-center justify-center gap-3 active:scale-95">
