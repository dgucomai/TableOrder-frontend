...
export default function OrderPage() {
  ...
  const fetchMenus = async () => {
    try {
      // 백엔드 주소로 GET 요청 (CORS 프록시 설정 시 '/api/menus'로 축약 가능)
      ...
      if (result.success) {
        ...
        // 2) 받아온 메뉴 데이터를 기반으로 카테고리 탭(All, 안주, 주류 등) 동적 생성
        const uniqueCategories = Array.from(new Set(fetchedMenus.map((m: MenuItem) => m.categoryName)));
